        ...
        with theme.frame('- CONTACT LOG -'):
            ...
            with ui.row().classes("w-full no-wrap py-4"):
                ...
                ui.label("PARENT CONTACT LOG").classes(
                        "w-screen justify-center items-center text-lg "
                        "font-bold"
                        )
            ...
            with ui.row().classes("w-screen no-wrap"):
                ui.input(
                        label="Guardian Name", value="", on_change=lambda e: u_guardianName.set_value(e.value), ).classes(
                        "w-["
                        "200px]"
                        ).props('aria-label="Guardian Name"').tooltip("Guardian Name")
                ui.input(
                        label="Phone Number", value="", on_change=lambda e: u_phoneNumber.set_value(e.value), ).classes(
                        "w-["
                        "200px]"
                        ).props('aria-label="Phone Number"').tooltip("Phone Number")
                ui.input(
                        label="Email Address", value="", on_change=lambda e: u_emailAddress.set_value(e.value), ).classes(
                        "w-["
                        "200px]"
                        ).props('aria-label="Email Address"').tooltip("Email Address")
            with ui.row().classes("w-screen no-wrap py-4"):
                ui.label().classes("w-[50px]")
            ...
            with ui.row().classes("w-screen no-wrap"):
                ...
                ui.select(
                        options=["Phone", "Text", "In-Person", "Email"], value="", on_change=lambda e: u_contactMethod.set_value(e.value), ).classes("w-[240px]").props(
                        'aria-label="Contact Method"'
                        ).tooltip("Contact Method")
                ui.label("Contact Response").classes("w-[50px]")
                ui.select(
                        options=["Answered", "Left Message", "Unable to Leave Message", "Disconnected", ], value="", on_change=lambda e: u_contactResponse.set_value(e.value), ).classes(
                        "w-[240px]"
                        ).props('aria-label="Contact Response"').tooltip(
                        "Contact "
                        "Response"
                        )
            with ui.row().classes("w-screen no-wrap py-4"):
            ...
            with ui.row().classes("w-screen no-wrap"):
                ...
                ui.select(
                        options=["IEP Related", "Discipline Related", "Student Requested"], value="", on_change=lambda e: u_contactGeneral.set_value(e.value), ).classes(
                        "w-[240px]"
                        ).props(
                        'aria-label="Reason for Contact - General '
                        'Category"'
                        ).tooltip("Reason for Contact - General Category")
                ui.label("Specific Topic").classes("w-[50px]")
                ui.select(
                        options=["Schedule IEP Meeting", "IEP Team Follow-Up", "Collaborate on Student IEP Goals", "Progress Monitoring Update", ], value="", on_change=lambda e: u_contactSpecific.set_value(e.value), ).classes("w-[240px]").props(
            ...
            with ui.row().classes("w-screen no-wrap "):
                ui.textarea(
                        label="Contact Notes (if email please copy/paste "
                              "email "
                              "into "
                              "the box)", value="", on_change=lambda e: u_contactNotes.set_value(e.value), ).classes("w-[640px]").props(
