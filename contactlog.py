...
if os.name == "nt":
    try:
        tmppath = Path(os.environ["USERPROFILE"]).joinpath(
                "Documents"
                )
...
for name in students:
    ...
    if (
        ...
        with open(
                tmppath, "a", newline="", encoding="UTF-8"
                ) as f_object:
            writer_setup = writer(f_object)
            writer_setup.writerow(list_names)
            f_object.close()
    ...
    with ui.tab_panel("CONTACT LOG"):
        ...
        with ui.row().classes("w-screen no-wrap"):
            ui.input(
                    label="Guardian Name",
                    value="",
                    on_change=lambda e: u_guardianName.set_value(
                            e.value
                            ),
            ...
            ui.input(
                    label="Phone Number",
                    value="",
                    on_change=lambda e: u_phoneNumber.set_value(
                            e.value
                            ),
                    ).classes("w-[200px]").props(
            ...
            ui.input(
                    label="Email Address",
                    value="",
                    on_change=lambda e: u_emailAddress.set_value(
                            e.value
                            ),
                    ).classes("w-[200px]").props(
        ...
        with ui.row().classes("w-screen no-wrap"):
            ...
            ui.select(
                    ...
                    value="",
                    on_change=lambda e: u_contactMethod.set_value(
                            e.value
                            ),
            ...
            ui.select(
                    ...
                    value="",
                    on_change=lambda e: u_contactResponse.set_value(
                            e.value
                            ),
                    ).classes("w-[240px]").props(
        ...
        with ui.row().classes("w-screen no-wrap"):
            ...
            ui.select(
                    options=["IEP Related", "Discipline Related",
                             "Student Requested"],
                    value="",
                    on_change=lambda e: u_contactGeneral.set_value(
                            e.value
                            ),
                    ).classes("w-[240px]").props(
            ...
            ui.select(
                    ...
                    value="",
                    on_change=lambda e: u_contactSpecific.set_value(
                            e.value
                            ),
                    ).classes("w-[240px]").props(
        ...
        with ui.row().classes("w-screen no-wrap "):
            ui.textarea(
                    label="Contact Notes (if email please copy/paste email "
                          ...
                          "the box)",
                    value="",
                    on_change=lambda e: u_contactNotes.set_value(
                            e.value
                            ),
    ...
    with ui.tab_panel("ABACUS SKILLS"):
        ...
        def graph(event):
            ...
            df_sql = pd.read_sql_query(
                    "SELECT * FROM ABACUSPROGRESS", conn
                    )
...
with ui.tab_panels(tabs, value="BRAILLE SKILLS"):
    with ui.tab_panel("BRAILLE SKILLS"):
        ...
        def graph(event):
            ...
            conn = sqlite3.connect(dataBasePath)
            df_sql = pd.read_sql_query(
                    "SELECT * FROM BRAILLEPROGRESS", conn
                    )
            df_student = df_sql[df_sql.STUDENTNAME == studentname]
            print(df_student)
            conn.close()
...
with ui.left_drawer(value=True).classes("bg-blue-950") as left_drawer:
    ...
    with ui.row().classes("w-full no-wrap py-4"):
        ui.link(
                ...
                "text-left w-screen text-white align-left font-bold"
                )
    with ui.row().classes("w-full no-wrap py-4"):
        ui.link(
                "Generic Data Sheets",
...
for monitor in get_monitors():
    print(
            f"Screen Resolution = {str(monitor.width)}x"
            f"{str(monitor.height)}"
            )
##############################################################################
# RUN CALL
##############################################################################
