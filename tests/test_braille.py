...
class Testbraille(unittest.TestCase):
    ...
    def test_graph(self, mock_make_subplots, mock_go, mock_np, mock_pd, mock_sqlite3):
        ...
        mock_sqlite3.connect.assert_called_once_with(Path(USER_DIR).joinpath("StudentDatabase", "students.db"))
        mock_pd.read_sql_query.assert_called_once_with("SELECT * FROM BRAILLEPROGRESS", mock_conn)
        mock_df_sql.__getitem__.assert_called_once_with(mock_df_sql.STUDENTNAME == "TestStudent")
