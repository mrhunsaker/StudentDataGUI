# FILEPATH: /c:/Users/RYHUNSAKER/GitHubRepos/StudentDataGUI/tests/test_digitalliteracy.py

import unittest
from pathlib import Path
from unittest.mock import patch, MagicMock
from appPages.digitalliteracy import save


class Testdigitalliteracy(unittest.TestCase):
    def setUp(self):
        # Mocking the UI elements
        self.u_studentname = MagicMock()
        self.u_today_date = MagicMock()
        self.u_digitalliteracy_trial11 = MagicMock()
        # ... repeat for all the other UI elements

    @patch("appPages.digitalliteracy.datetime")
    @patch("appPages.digitalliteracy.Path")
    @patch("appPages.digitalliteracy.json")
    def test_save(self, mock_json, mock_path, mock_datetime):
        ...
        self.u_digitalliteracy_trial11.value = 1
        # ... repeat for all the other UI elements
